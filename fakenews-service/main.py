...
# ══════════════════════════════════════════════════════════
#  FastAPI wrapper  —  fakenews-service
#  Exposes:  GET /health   POST /detect
#  Body:     { "content": "..." }  OR  { "url": "https://..." }
# ══════════════════════════════════════════════════════════
...
async def lifespan(app: FastAPI):
    log.info("fakenews-service ready  model=%s  pipeline=%s",
             MODEL_ID, "loaded" if _pipeline else "heuristic-only")
    yield


app = FastAPI(title="Fake News Detection Service", version="1.0.0", lifespan=lifespan)
...
async def detect(req: FakeNewsRequest):
    # ── Resolve text from content or URL ──
    if req.url:
        try:
